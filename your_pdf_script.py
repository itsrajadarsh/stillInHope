...
def insert_image_and_text_into_pdf(pdf_path, image_path, output_path,
    ...
    cropped_image_path = crop_resize_and_compress_image(image_path, width, height)
...
def crop_resize_and_compress_image(image_path, target_width, target_height, output_temp_path="temp_cropped_image.jpg", target_size_kb=500):
    with Image.open(image_path) as img:
        img_width, img_height = img.size
        target_ratio = target_width / target_height
        img_ratio = img_width / img_height
        ...
        # 🔹 Step 2: Resize to match PDF placement size
        resized_img = cropped_img.resize((target_width, target_height), Image.LANCZOS)

        # 🔹 Step 3: Compress until <= target_size_kb
        quality = 95
        while quality > 60:
            resized_img.save(output_temp_path, format="JPEG", quality=quality, optimize=True)
            if os.path.getsize(output_temp_path) <= target_size_kb * 1024:
                break
